...
public class DungeonManager : MonoBehaviour
{
    ...
    //Список с врагами для сохранения
    private List<PreSaveEntityData> enemies;
    //Список с интерактивными объектами для сохранения
    private List<PreSaveEntityData> interactables;
    //Семя уровня
    private int seed;
    //Текущий уровень для сохранения
    ...
    public class PreSaveEntityData
    {
        public int arrayIndex;
        public GameObject entity;

        public PreSaveEntityData(int arri, GameObject go)
        {
            arrayIndex = arri;
            entity = go;
        }
    ...
    //Создать игрока
    private void PlacePlayer(SubDungeon smallestRoom)
    {
    ...
    //Флаг blockPath дополнительно делает место непроходимым.
    //Флаг spawnInStartRoom определяет можно ли выбрать позицию для установки объекта в стартовой комнате (чтобы в ней не появились враги).
    //Флаг generateEd определяет создавать ли список с объектами для сохранения
    private List<PreSaveEntityData> LayoutObjectsAtRandom(GameObject[] tileArray, int minimum, int maximum, bool blockPath, bool spawnInStartRoom, bool generateEd)
    {
        List<PreSaveEntityData> entities = new List<PreSaveEntityData>();
    ...
    //Получение файла с данными для сохранения
    public SaveData GetSaveData()
    {
        ...
        sd.worldSeed = seed;
        sd.playerHealth = player.getHitPoints();
        sd.playerX = playerGo.transform.position.x;
        sd.PlayerY = playerGo.transform.position.y;
        sd.score = GameManager.instance.GetScore();
        sd.level = currentLevel;

        sd.enemyData = new List<EntityData>();
        foreach (PreSaveEntityData psed in enemies)
        {
            if (psed.entity.activeSelf)
            {
                EntityData ed = new EntityData();
                ed.arrayIndex = psed.arrayIndex;
                ed.x = psed.entity.transform.position.x;
                ed.y = psed.entity.transform.position.y;
                sd.enemyData.Add(ed);
            }
        }

        sd.interactablesData = new List<EntityData>();
        foreach (PreSaveEntityData psed in interactables)
        {
            if (psed.entity.activeSelf)
            {
                EntityData ed = new EntityData();
    ...
    public void NewDungeon(int level)
    {
        seed = Random.Range(0, int.MaxValue);
        Random.InitState(seed);

        SetupScene(level, false);
    }

    //Создание уровня из сохраненных данных
    public void LoadDungeon(SaveData sd)
    {
        seed = sd.worldSeed;
        Random.InitState(seed);

        SetupScene(sd.level, true);

        Instantiate(tiles[activeTileset].playerTile, new Vector3(sd.playerX, sd.PlayerY, 0f), Quaternion.identity);
        interactables = new List<PreSaveEntityData>();
        foreach (EntityData ed in sd.interactablesData)
        {
            GameObject instance = Instantiate(tiles[activeTileset].interactableTiles[ed.arrayIndex], new Vector3(ed.x, ed.y, 0f), Quaternion.identity);
            interactables.Add(new PreSaveEntityData(ed.arrayIndex, instance));
        }
        enemies = new List<PreSaveEntityData>();
        foreach (EntityData ed in sd.enemyData)
        {
            GameObject instance = Instantiate(tiles[activeTileset].enemyTiles[ed.arrayIndex], new Vector3(ed.x, ed.y, 0f), Quaternion.identity);
            enemies.Add(new PreSaveEntityData(ed.arrayIndex, instance));
        }
    }

    //Начать создание уровня
    public void SetupScene(int level, bool load)
    {
        currentLevel = level;
        ...
        SubDungeon smallestRoom = SmallestRoom();
        if (!load)
        {
            PlacePlayer(smallestRoom);
        }
        PlaceExit();

        blocksCount = new Count(freePositions.Count / 12, freePositions.Count / 8);
        LayoutObjectsAtRandom(tiles[activeTileset].blockTiles, blocksCount.minimum, blocksCount.maximum, true, true, false);
        int trapCount = (int)Mathf.Clamp(Mathf.Pow(1.8f, level), 20, freePositions.Count / 13f);
        LayoutObjectsAtRandom(tiles[activeTileset].trapTiles, trapCount, trapCount, true, false, false);

        if (!load)
        {
            interactables = LayoutObjectsAtRandom(tiles[activeTileset].interactableTiles, interactablesCount.minimum, interactablesCount.maximum, false, false, true);
            int enemyCount = (int)Mathf.Clamp(Mathf.Pow(2f, level), 18, freePositions.Count / 10f);
            enemies = LayoutObjectsAtRandom(tiles[activeTileset].enemyTiles, enemyCount, enemyCount, false, false, true);
        }
    }
}